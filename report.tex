...
\hline
1 & Nguyễn Nhật Tân & 2213059 & Table Module, Architecture lead và report\\
\hline
2 & Lê Bảo Linh & 2311849 & Order Module \\
\hline
3 & Trần Phú & 1952924 & Billing/Payment Module \\
\hline
4 & Phạm Đình Chương & 2210400 & Testing và doc\\
\hline
5 & Trần Minh Đăng & 2310735 & Kitchen Module \\
\hline
6 & Hà Đình Vinh & 2313907 & Frontend và integration\\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Containerization}: Docker Compose cho frontend, backend và PostgreSQL.
\end{itemize}
...
Cấu trúc thư mục chính của IRMS như sau:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/login1.png}
    \caption{Màn hình đăng nhập}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/login2.png}
    \caption{Màn hình đăng nhập của quản trị viên}
\end{figure}

\subsubsection{Màn hình quản lý}

Màn hình quản lý đóng vai trò như một bảng điều khiển tổng quan dành cho quản lý nhà hàng. Tại đây, người dùng có thể theo dõi các thống kê chung như doanh thu theo ngày hoặc theo ca, danh sách các món bán chạy và các đơn hàng vừa hoàn tất để nắm nhanh tình hình vận hành. Giao diện này giúp nhà quản lý quan sát hiệu quả kinh doanh, nhận diện xu hướng tiêu thụ món ăn và kiểm tra các giao dịch mới nhất mà không cần đi sâu vào từng module nghiệp vụ riêng lẻ.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/manager.png}
    \caption{Màn hình quản lý}
\end{figure}

\subsubsection{Màn hình quản trị}

Màn hình quản trị dành cho quản trị viên hoặc quản lý cấp cao, đóng vai trò là khu vực điều hành tập trung của toàn hệ thống. Tại đây, người dùng có thể truy cập các chức năng tổng quan, phân tích doanh thu và quản lý tập trung nhiều nhóm dữ liệu quan trọng như nhân sự, menu, nguyên liệu, bàn và đơn hàng. Giao diện này giúp theo dõi hoạt động toàn cục, kiểm soát dữ liệu vận hành cốt lõi và hỗ trợ các quyết định quản trị dựa trên số liệu thay vì chỉ thao tác ở từng nghiệp vụ riêng lẻ.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin1.png}
    \caption{Màn hình quản trị với chức năng tổng quan}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin2.png}
    \caption{Màn hình quản trị với chức năng phân tích doanh thu}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin3.png}
    \caption{Màn hình quản trị cho quản lý tập trung menu}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin4.png}
    \caption{Màn hình quản trị cho quản lý tập trung nhân sự}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin5.png}
    \caption{Màn hình quản trị cho quản lý nguyên liệu}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin6.png}
    \caption{Màn hình quản trị cho quản lý bàn}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/admin7.png}
    \caption{Màn hình quản trị cho quản lý đơn hàng}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/reception1.png}
    \caption{Màn hình lễ tân cho đặt bàn và giữ bàn}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/reception2.png}
    \caption{Màn hình lễ tân cho xử lý khách walk-in}
\end{figure}

\subsubsection{Màn hình phục vụ bàn}

Màn hình phục vụ bàn hỗ trợ nhân viên phục vụ thao tác trực tiếp trên từng bàn đang có khách. Ở tầng backend, trạng thái bàn được lưu ổn định dưới các giá trị \texttt{AVAILABLE}, \texttt{OCCUPIED}, \texttt{RESERVED} và \texttt{CLEANING}. Tại đây, người dùng có thể xem tình trạng bàn, theo dõi order đang mở, duyệt menu để chọn món, thêm số lượng, nhập ghi chú và gửi yêu cầu xuống hệ thống. Giao diện này tập trung các thao tác cần thiết trong quá trình phục vụ tại bàn, đồng thời cho phép gọi thêm món nhiều lần trước khi thanh toán; các lần gọi mới tiếp tục được gắn vào order đang mở và phát sinh ticket tương ứng cho bếp.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/server1.png}
    \caption{Màn hình phục vụ bàn với theo dõi tình trạng bàn}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/server2.png}
    \caption{Màn hình phục vụ bàn với thao tác gọi món}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/server4.png}
    \caption{Màn hình duyệt menu}
\end{figure}

\subsubsection{Màn hình quản lý bếp và kho}

Màn hình quản lý bếp và kho hỗ trợ nhân viên theo dõi các ticket chế biến đang chờ xử lý, tình trạng món đang chuẩn bị và lịch sử các món đã hoàn thành. Bên cạnh đó, giao diện này còn cho phép cập nhật tình trạng nguyên liệu trong kho, ghi nhận nguyên liệu sắp hết hoặc đã hết để phối hợp giữa khu vực bếp và bộ phận quản lý. Việc kết hợp quản lý bếp với kho giúp hệ thống phản ánh tốt hơn năng lực phục vụ thực tế và hỗ trợ điều chỉnh menu hoặc kế hoạch chế biến kịp thời.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/chef1.png}
    \caption{Màn hình quản lý các món đang chờ chế biến}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/chef2.png}
    \caption{Màn hình quản lý kho}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/chef3.png}
    \caption{Màn hình tình trạng thực đơn}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{frontend_img/server3.png}
    \caption{Màn hình lập bill và thanh toán}
\end{figure}
...
\begin{itemize}
    ...
    \item Màn hình quản lý với thống kê tổng quan
    \item Màn hình quản trị với quản lý tập trung và phân tích doanh thu
    \item Màn hình lễ tân cho đặt bàn, giữ bàn và khách walk-in
    \item Màn hình phục vụ bàn của nhân viên phục vụ
    \item Màn hình quản lý bếp và kho với ticket chế biến
    \item Màn hình quản lý bếp và kho với cập nhật tình trạng nguyên liệu
...
Mã nguồn của dự án được quản lý tại \href{https://github.com/KeHamTruyen/IRMS}{repository GitHub IRMS}. Repository này phục vụ cho việc theo dõi tiến độ, cộng tác giữa các thành viên và đối chiếu với phần hiện thực được mô tả trong báo cáo.

Hệ thống có thể được đóng gói bằng Docker để đơn giản hóa quá trình triển khai và demo:
\begin{itemize}
    \item Backend được đóng gói bằng \texttt{backend/Dockerfile}: Maven build tạo file \texttt{jar}, sau đó chạy bằng Java runtime image.
    \item Frontend được đóng gói bằng \texttt{frontend/Dockerfile}: Vite build tạo static assets và Nginx phục vụ giao diện.
    \item PostgreSQL, backend và frontend được điều phối bằng \texttt{docker-compose.yml} ở thư mục gốc dự án.
    \item Nginx trong frontend proxy các request \texttt{/api} đến backend container, giúp demo truy cập qua \texttt{localhost:5173}.
    \item Flyway migration tạo schema và seed dữ liệu mẫu cho tài khoản, bàn, menu, order, bill và kitchen order.
\end{itemize}

Ví dụ cấu hình chạy:
\begin{itemize}
    \item Lệnh chạy: \texttt{docker compose up --build}
    \item Backend: \texttt{localhost:3000}
    \item Frontend: \texttt{localhost:5173}
    \item Database: PostgreSQL container, mapped ra \texttt{localhost:5432}
\end{itemize}
