...
Hệ thống \textbf{Intelligent Restaurant Management System (IRMS)} là một giải pháp phần mềm nhằm số hóa và tối ưu hóa các hoạt động cốt lõi trong nhà hàng, bao gồm tiếp nhận order, điều phối bếp, quản lý bàn, lập hóa đơn và thanh toán. Hệ thống đóng vai trò như một nền tảng trung tâm giúp đồng bộ thông tin giữa nhân viên phục vụ, nhân viên bếp, thu ngân và quản lý, từ đó giảm sai sót, rút ngắn thời gian phục vụ và nâng cao trải nghiệm khách hàng.
...
\begin{itemize}
    ...
    \item Cải thiện khả năng phối hợp giữa nhân viên phục vụ, nhân viên bếp và thu ngân thông qua dữ liệu đồng bộ theo thời gian thực.
...
\begin{itemize}
    ...
    \item \textbf{Billing \& Payment}: tạo hóa đơn từ order, tính tổng tiền, áp dụng khuyến mãi đơn giản và xử lý thanh toán.
...
\begin{itemize}
    ...
    \item \textbf{Cashier}: tạo bill, xử lý thanh toán và in hoặc xác nhận hóa đơn.
    \item \textbf{Manager}: quản lý menu, theo dõi hoạt động vận hành, cập nhật giá và xem báo cáo cơ bản.
    \item \textbf{Host}: điều phối khách, quản lý khu vực bàn và hỗ trợ phân bổ bàn; reservation là hướng mở rộng.
...
\begin{itemize}
    ...
    \item Mỗi bàn có thể ở một trong các trạng thái: \texttt{Available}, \texttt{Occupied}, \texttt{Reserved}, \texttt{WaitingForPayment}, \texttt{Dirty}.
...
\begin{itemize}
    \item Thu ngân hoặc nhân viên phục vụ có thể tạo bill từ order đang hoạt động.
...
\begin{itemize}
    \item Hệ thống phải hỗ trợ các vai trò: \texttt{Admin}, \texttt{Manager}, \texttt{Server}, \texttt{Chef}, \texttt{Cashier}, \texttt{Host}.
...
\begin{itemize}
    ...
    \item \textbf{Khả năng mở rộng}: dễ dàng bổ sung các module như Reservation, Employee Scheduling hoặc Loyalty Program trong tương lai.
...
\begin{itemize}
    ...
    \item \textbf{Extensibility}: kiến trúc cần cho phép thêm các module như Reservation, Loyalty hoặc Employee Scheduling mà không phá vỡ cấu trúc lõi.
...
\begin{itemize}
    ...
    \item \textbf{Extensibility}: hệ thống cần hỗ trợ mở rộng các module như Reservation, Employee Scheduling hoặc Loyalty Program trong tương lai.
...
\hline
Cashier & generateBill, processPayment, printReceipt, closeOrder \\
\hline
Manager & manageMenu, updatePrice, viewOperations, viewBasicReports, manageUsers \\
\hline
Host & assignTable, manageReservations (planned) \\
\hline
...
Trong phiên bản hiện tại, Inventory và Analytics đã có backend cơ bản; Reservation và Audit Log vẫn nằm trong kế hoạch mở rộng.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item services: \texttt{IKitchenService}, \texttt{KitchenServiceImpl}, \texttt{KitchenOrderTimeoutService}
...
\begin{enumerate}
    ...
    \item Cashier UI thực hiện thanh toán.
...
\begin{itemize}
    ...
    \item \textbf{Client Node 3}: Terminal cho thu ngân.
...
\begin{itemize}
    \item Thêm \textbf{Reservation Module} để quản lý đặt chỗ trước.
...
Ví dụ, \texttt{Reservation Module} có thể kết nối với \texttt{Table Module} để tối ưu phân bổ bàn theo khung giờ, trong khi \texttt{Loyalty Module} có thể lấy dữ liệu từ Bill và Order để tính điểm thưởng mà không làm ảnh hưởng trực tiếp tới các module lõi hiện tại.
...
Các module Reservation, Employee Scheduling, Loyalty và Audit Log vẫn ở mức thiết kế hoặc kế hoạch mở rộng, chưa có module riêng hoàn chỉnh trong mã nguồn hiện tại.
...
\begin{itemize}
    ...
    \item \textbf{Frontend}: ReactJS, TypeScript, Vite, Axios.
...
Ví dụ cấu trúc thư mục cho backend IRMS như sau:
...
Màn hình quản lý bàn hiển thị trạng thái hiện tại của từng bàn, cho phép nhân viên phục vụ hoặc host chọn bàn phù hợp trước khi tạo order. Các trạng thái như Available, Occupied hoặc WaitingForPayment được thể hiện rõ ràng để hỗ trợ quyết định nhanh trong môi trường phục vụ thực tế.

\subsubsection{Màn hình gọi món}

Màn hình gọi món cho phép nhân viên phục vụ chọn món từ menu, thêm số lượng, nhập ghi chú và gửi order đến hệ thống. Sau khi order được xác nhận, dữ liệu được lưu lại và chuyển tiếp đến Kitchen Module.

\subsubsection{Màn hình bếp}

Màn hình bếp hiển thị danh sách kitchen ticket đang chờ xử lý. Nhân viên bếp có thể cập nhật trạng thái từng món hoặc từng ticket, từ đó hệ thống phản ánh lại trạng thái order cho khu vực phục vụ.

\subsubsection{Màn hình lập bill và thanh toán}

Màn hình bill hỗ trợ tổng hợp món đã gọi, tính subtotal, service fee, discount và total. Thu ngân có thể lựa chọn phương thức thanh toán phù hợp và xác nhận giao dịch. Sau khi thanh toán thành công, trạng thái bill và order được cập nhật đồng bộ.
...
\begin{enumerate}
    ...
    \item Thu ngân tạo bill và xử lý thanh toán.
...
Khó khăn lớn nhất của nhóm là xác định ranh giới module sao cho vừa đúng với domain, vừa không bị dàn trải. Ban đầu, một số trách nhiệm như quản lý bàn, order và billing có xu hướng bị gộp vào chung một service, làm tăng độ phức tạp. Ngoài ra, việc áp dụng DIP và ISP khiến thiết kế ban đầu mất nhiều thời gian hơn vì nhóm phải suy nghĩ kỹ về abstraction thay vì code trực tiếp.
...
\hline
5 & Trần Minh Đăng & 2310735 & Frontend và integration: màn hình demo cho server, kitchen, cashier; tích hợp API của các module chính \\
\hline
