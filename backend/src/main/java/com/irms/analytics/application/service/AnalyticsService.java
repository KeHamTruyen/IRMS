...
public class AnalyticsService {
        ...
        public SalesReportResponse getSalesReport(LocalDate startDate, LocalDate endDate) {
                ...
                List<com.irms.order.domain.entity.Order> orders = orderRepository.findByCreatedAtBetween(startDateTime,
                                endDateTime);
                ...
                List<BestSellingItemResponse> bestSellingItems = orderItemRepository
                                .findTopSellingItems(startDateTime, endDateTime)
                                .stream()
        ...
        public String exportSalesReportCsv(LocalDate startDate, LocalDate endDate) {
                ...
                for (BestSellingItemResponse item : report.getBestSellingItems()) {
                        csv.append(item.getMenuItemId()).append(',')
                                        .append('"').append(item.getItemName().replace("\"", "\"\"")).append('"')
                                        .append(',')
        ...
        public RevenueAnalyticsResponse getRevenueAnalytics() {
                ...
                BigDecimal todayRevenue = revenueFor(today, today);
                BigDecimal yesterdayRevenue = revenueFor(yesterday, yesterday);
                BigDecimal thisWeekRevenue = revenueFor(weekStart, today);
                BigDecimal lastWeekRevenue = revenueFor(lastWeekStart, lastWeekStart.plusDays(6));
                BigDecimal thisMonthRevenue = revenueFor(monthStart, today);
                BigDecimal lastMonthRevenue = revenueFor(lastMonthStart, lastMonthEnd);

                List<RevenueAnalyticsResponse.RevenueComparison> comparisons = List.of(
                                comparison("today", "Today", todayRevenue, yesterdayRevenue),
                                comparison("week", "This week", thisWeekRevenue, lastWeekRevenue),
                                comparison("month", "This month", thisMonthRevenue, lastMonthRevenue));
                ...
                return RevenueAnalyticsResponse.builder()
                                ...
                                .weeklyTrend(buildDailyTrend(weekStart, today, lastWeekStart))
                                .monthlyTrend(buildDailyTrend(monthStart, today, lastMonthStart))
                                .build();
        }

        private BigDecimal revenueFor(LocalDate startDate, LocalDate endDate) {
                Double value = billRepository.getTotalRevenue(
                                LocalDateTime.of(startDate, LocalTime.MIN),
                                LocalDateTime.of(endDate, LocalTime.MAX));
                return value != null ? BigDecimal.valueOf(value) : BigDecimal.ZERO;
        }

        private RevenueAnalyticsResponse.RevenueComparison comparison(
                        String id,
                        String label,
                        BigDecimal current,
                        BigDecimal previous) {
        ...
        private List<RevenueAnalyticsResponse.RevenueTrendPoint> buildDailyTrend(
                        LocalDate currentStart,
                        LocalDate currentEnd,
                        LocalDate previousStart) {
                List<RevenueAnalyticsResponse.RevenueTrendPoint> points = new ArrayList<>();
                LocalDate cursor = currentStart;
                int offset = 0;

                while (!cursor.isAfter(currentEnd)) {
                        LocalDate previousDate = previousStart.plusDays(offset);
                        points.add(RevenueAnalyticsResponse.RevenueTrendPoint.builder()
                                        .label(cursor.toString())
                                        .current(revenueFor(cursor, cursor))
                                        .previous(revenueFor(previousDate, previousDate))
                                        .build());
